...
describe("AdminUserAPI", () => {
    ...
    it("should collect all admin users by walking through the pages", async () => {
        mock.onGet("").reply((config) => {
            ...
            expect(config.params.size).toBe(200);
            return [200, {
    ...
    it("should find a sorted and searched page of admin users", async () => {
        ...
        expect(mock.history.get[0]?.params).toEqual({page: 0, size: 10, sort_by: "last_name", direction: "ASC", search: "ada", case_sensitive: false});
