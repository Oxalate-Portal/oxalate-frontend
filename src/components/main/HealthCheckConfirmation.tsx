...
interface HealthCheckConfirmationProps {
    registration: boolean;
}

export function HealthCheckConfirmation({registration}: HealthCheckConfirmationProps) {
    const {userSession, logoutUser, refreshUserSession} = useSession();
    const {t} = useTranslation();
    const [error, setError] = useState<string | null>(null);

    async function confirmHealthCheck(answer: string) {
        const payload = {healthCheckAnswer: answer};

        userAPI.confirmHealthCheck(payload)
                .then((response) => {
    ...
    if (error !== null) {
        return (
                <Alert type={"error"}
                       title={t("HealthCheckConfirmation.error.alert")}/>
        );
    }
            ...
            <div style={{textAlign: "center"}} className={"darkDiv"}>
                <div style={{width: "50%", display: "inline-block", textAlign: "left"}}>
                    ...
                    <Space orientation={"vertical"} size={20} style={{width: "100%", margin: 12}}>
                        <Page pageId={3} showTitle={false} showDate={false}/>

                        {!registration &&
                                <Space orientation={"horizontal"} size={12} style={{width: "100%", justifyContent: "center"}}>
                                    <Button type={"primary"}
                                            onClick={() => confirmHealthCheck("yes")}>{t("HealthCheckConfirmation.button.confirm")}</Button>
                                    <Button danger={true} type={"primary"} href={"/user"}
                                            onClick={() => confirmHealthCheck("no")}>{t("HealthCheckConfirmation.button.reject")}</Button>
